...
<body>
    <div class="pt-3 bg-darkGrey  position-relative header">
        <div class="container  pt-2 pb-3 text-white d-flex">
            ...
            <h1 class="fs-3 fw-bold">Tambah Data Rute Penerbangan</h1><i class="bi bi-airplane-fill ms-2" style="color: #FFD523;"></i>
        </div>
    ...
    <div class="container mt-4 form-add">
        <form class="col-md-8" method="post" action="">
            <!-- input nama maskapai -->
            <div class="mb-4">
                <input type="text" placeholder="Nama Maskapai" name="nama_maskapai" required>
            </div>


            <!-- bandara asal -->
            <div class="mb-4">
                <select class="form-select form-select rounded-0" aria-label="Small select example" required name="bandara_asal">
                    <option value="" hidden>Bandara Asal</option>
                    <?php foreach ($dataAsal as $data) { ?>
                        <option value="<?= $data['id'] ?>"><?= $data['nama_bandara'] ?></option>
                    <?php } ?>
                </select>
            </div>
            ...
            <!-- bandara tujuan -->
            <div class="mb-4">
                <select class="form-select form-select rounded-0" aria-label="Small select example" required name="bandara_tujuan">
            ...
            <button type="submit" class="btn bg-darkGrey text-white btn-primary" name="btn_submit">Submit</button>
        </form>
    </div>
